...
public class CustomerCont implements Initializable {
     ...
     * The lambda in fewer lines implements what a for loop iterating through an entire array would accomplish.
    ...
    void countryCBAction(ActionEvent event) {
        if (!isUpdating.get()){
            ...
            allCountries.stream()
                    .filter(country -> country.equals(countryCB.getSelectionModel().getSelectedItem()))
                    .findFirst()
                    .ifPresent(country -> selectedCountry = country);
     ...
     * It then uses a stream operation to filter the allDivisions list based on the selected item.
     * The findFirst() method retrieves the first matching division, if any, and sets it as the selectedDivision using the ifPresent() method.
     *
    ...
    void divisionCBAction(ActionEvent event) {
        if (!isUpdating.get()){
            isUpdating.set(true);
            allDivisions.stream()
                    .filter(division -> division.equals(divisionCB.getSelectionModel().getSelectedItem()))
                    .findFirst()
                    .ifPresent(div -> selectedDivision = div);
        }
        isUpdating.set(false);
    }
     ...
     * Using a lambda this method effectively cuts down the amount of code needed for its function.
     * The lambda implements the ability of the listener to select Customers objects.
